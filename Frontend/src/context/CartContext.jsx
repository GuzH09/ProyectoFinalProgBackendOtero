...
export const CartProvider = ({ children }) => {
  ...
  const fetchCartFromUser = useCallback(async () => {
    try {
      const responseCart = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/carts/${profile.cart[0]}`, {
        credentials: 'include'
      })
  ...
  const handleOnRemove = async (id) => {
    try {
      await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/carts/${profile.cart[0]}/product/${id}/delete`, {
        method: 'POST',
        headers: {
  ...
  const handleOnAdd = async (quantity, id, title) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/carts/${profile.cart[0]}/product/${id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (response.ok) {
        const responseQuantity = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/carts/${profile.cart[0]}/product/${id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  const handleOnCheckout = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/carts/${profile.cart[0]}/purchase/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
